...
namespace p1sh {
	GLuint createShaderProgram(const char * vertex_file_path, const char * fragment_file_path) {
		...
		printf("Create shader success.Program ID:%u\nvs:%s,fs:%s\n",ProgramID, vertex_file_path, fragment_file_path);
	...
	void ShaderManager::initAllShaders() {
		...
		_shaderHash[ShaderType::ST_MVP] = createShaderProgram("./shaders/mvpVS.vs", "./shaders/mvpFS.fs");
